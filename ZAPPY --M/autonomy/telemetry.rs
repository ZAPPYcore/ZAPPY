...
/// Builder for configuring autonomy telemetry sinks.
pub struct AutonomyTelemetryBuilder {
    module: String,
    log_path: Option<PathBuf>,
    event_publisher: Option<Arc<dyn EventPublisher>>,
}

impl AutonomyTelemetryBuilder {
    /// Creates a builder for the specified module name.
    #[must_use]
    ...
    /// Registers the event publisher to use.
    #[must_use]
    ...
    pub fn build(self) -> Result<AutonomyTelemetry> {
        AutonomyTelemetry::new(self.module, self.log_path, self.event_publisher)
    }
}

/// Telemetry handle shared across autonomy subsystems.
#[derive(Clone)]
pub struct AutonomyTelemetry {
    inner: Arc<AutonomyTelemetryInner>,
}

impl fmt::Debug for AutonomyTelemetry {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("AutonomyTelemetry")
            .field("module", &self.inner.module)
            .finish()
    }
}

struct AutonomyTelemetryInner {
    module: String,
...
impl AutonomyTelemetry {
    fn new(
        ...
        Ok(Self {
            inner: Arc::new(AutonomyTelemetryInner {
                module: module.into(),
                logger,
                event,
            }),
    ...
    /// Returns a builder for convenience.
    #[must_use]
    pub fn builder(module: impl Into<String>) -> AutonomyTelemetryBuilder {
        AutonomyTelemetryBuilder::new(module)
    }
    ...
    /// Emits an event record to the configured bus.
...
mod tests {
    ...
    fn telemetry_logs_and_emits() {
        ...
        let log_path = dir.path().join("autonomy.log");
        let bus = Arc::new(MemoryEventBus::new(8));
        let telemetry = AutonomyTelemetry::builder("autonomy")
            .log_path(&log_path)
        ...
        telemetry
            ...
            .unwrap();
        telemetry
            .event("autonomy.decision.started", json!({ "cycle": 1 }))
            .unwrap();
        assert!(std::fs::read_to_string(log_path)
            .unwrap()
            .contains("decision_cycle_start"));
