...
/// Builder configuring telemetry sinks for the metacognition runtime.
pub struct MetacognitionTelemetryBuilder {
    module: String,
    log_path: Option<PathBuf>,
    event_publisher: Option<Arc<dyn EventPublisher>>,
}

impl MetacognitionTelemetryBuilder {
    /// Creates a new builder for the provided module label.
    #[must_use]
    ...
    /// Builds the telemetry helper.
    pub fn build(self) -> Result<MetacognitionTelemetry> {
        MetacognitionTelemetry::new(self.module, self.log_path, self.event_publisher)
    }
}

/// Telemetry handle shared by metacognition runtime and kernel.
#[derive(Clone)]
pub struct MetacognitionTelemetry {
    inner: Arc<TelemetryInner>,
}

impl fmt::Debug for MetacognitionTelemetry {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("MetacognitionTelemetry")
...
impl MetacognitionTelemetry {
    ...
    #[must_use]
    pub fn builder(module: impl Into<String>) -> MetacognitionTelemetryBuilder {
        MetacognitionTelemetryBuilder::new(module)
    }
...
mod tests {
    ...
    fn telemetry_logs_and_emits() {
        ...
        let log_path = dir.path().join("metacognition.log");
        let bus = Arc::new(MemoryEventBus::new(8));
        let telemetry = MetacognitionTelemetry::builder("metacognition")
            .log_path(&log_path)
            .event_publisher(bus.clone())
            .build()
            .unwrap();
        telemetry
            .log(LogLevel::Info, "reflection_start", json!({ "cycle": 1 }))
            .unwrap();
        telemetry
            .event("metacognition.reflection.started", json!({ "cycle": 1 }))
            .unwrap();
        let content = std::fs::read_to_string(log_path).unwrap();
        assert!(content.contains("reflection_start"));
