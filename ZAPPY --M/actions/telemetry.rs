...
/// Builder that configures logging + event sinks for the action fabric.
pub struct ActionTelemetryBuilder {
    module: String,
    log_path: Option<PathBuf>,
    event_publisher: Option<Arc<dyn EventPublisher>>,
}

impl ActionTelemetryBuilder {
    /// Creates a new builder for the given module name.
    #[must_use]
    ...
    pub fn build(self) -> Result<ActionTelemetry> {
        ActionTelemetry::new(self.module, self.log_path, self.event_publisher)
    }
}

/// Shared telemetry handle for logging + event emission.
#[derive(Clone)]
pub struct ActionTelemetry {
    inner: Arc<ActionTelemetryInner>,
}

struct ActionTelemetryInner {
    module: String,
...
impl ActionTelemetry {
    fn new(
        ...
        Ok(Self {
            inner: Arc::new(ActionTelemetryInner {
                module: module.into(),
                logger,
                event,
            }),
    ...
    #[must_use]
    pub fn builder(module: impl Into<String>) -> ActionTelemetryBuilder {
        ActionTelemetryBuilder::new(module)
    }
...
mod tests {
    ...
    fn telemetry_logs_and_emits() {
        ...
        let log_path = dir.path().join("actions.log");
        let bus = Arc::new(MemoryEventBus::new(8));
        let telemetry = ActionTelemetry::builder("actions")
            .log_path(&log_path)
            .event_publisher(bus.clone())
            .build()
            .unwrap();
        telemetry
            .log(LogLevel::Info, "test_log", json!({ "foo": "bar" }))
            .unwrap();
        telemetry
            .event("actions.test", json!({ "baz": 1 }))
            .unwrap();
        let content = std::fs::read_to_string(log_path).unwrap();
        assert!(content.contains("test_log"));
