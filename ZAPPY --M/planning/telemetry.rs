...
/// Builder configuring telemetry sinks for the planning runtime.
pub struct PlanningTelemetryBuilder {
    module: String,
    log_path: Option<PathBuf>,
    event_publisher: Option<Arc<dyn EventPublisher>>,
}

impl PlanningTelemetryBuilder {
    ...
    pub fn build(self) -> Result<PlanningTelemetry> {
        PlanningTelemetry::new(self.module, self.log_path, self.event_publisher)
    }
}

/// Telemetry handle shared across long-term and short-term planners.
#[derive(Clone)]
pub struct PlanningTelemetry {
    inner: Arc<TelemetryInner>,
}

impl fmt::Debug for PlanningTelemetry {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("PlanningTelemetry")
...
impl PlanningTelemetry {
    ...
    /// Returns a builder.
    #[must_use]
    pub fn builder(module: impl Into<String>) -> PlanningTelemetryBuilder {
        PlanningTelemetryBuilder::new(module)
    }
    ...
    /// Emits an event to the configured bus.
...
mod tests {
    ...
    fn telemetry_logs_and_emits() {
        ...
        let log_path = dir.path().join("planning.log");
        let bus = Arc::new(MemoryEventBus::new(8));
        let telemetry = PlanningTelemetry::builder("planning")
            .log_path(&log_path)
        ...
        telemetry
            .log(
                LogLevel::Info,
                "planning.long_term.start",
                json!({ "objectives": 3 }),
            )
            .unwrap();
        telemetry
            .event("planning.schedule.generated", json!({ "tasks": 5 }))
            .unwrap();
        let content = std::fs::read_to_string(log_path).unwrap();
        assert!(content.contains("planning.long_term.start"));
