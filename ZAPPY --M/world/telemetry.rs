...
/// Builder for world telemetry.
pub struct WorldTelemetryBuilder {
    module: String,
    log_path: Option<PathBuf>,
    event_publisher: Option<Arc<dyn EventPublisher>>,
}

impl WorldTelemetryBuilder {
    /// Creates a builder.
    #[must_use]
    ...
    /// Builds telemetry handle.
    pub fn build(self) -> Result<WorldTelemetry> {
        WorldTelemetry::new(self.module, self.log_path, self.event_publisher)
    }
}

/// Telemetry handle used across world runtime.
#[derive(Clone)]
pub struct WorldTelemetry {
    inner: Arc<TelemetryInner>,
}

impl fmt::Debug for WorldTelemetry {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("WorldTelemetry")
...
impl WorldTelemetry {
    ...
    /// Returns builder.
    #[must_use]
    pub fn builder(module: impl Into<String>) -> WorldTelemetryBuilder {
        WorldTelemetryBuilder::new(module)
    }
...
mod tests {
    ...
    #[test]
    fn telemetry_logs_and_events() {
        let tmp = tempdir().unwrap();
        let path = tmp.path().join("world.log");
        let bus = Arc::new(MemoryEventBus::new(8));
        let telemetry = WorldTelemetry::builder("world")
            .log_path(&path)
            .event_publisher(bus.clone())
            .build()
            .unwrap();
        telemetry
            .log(LogLevel::Info, "world.update", json!({ "regions": 2 }))
            .unwrap();
        telemetry
            .event("world.alert", json!({ "severity": "high" }))
            .unwrap();
        let contents = std::fs::read_to_string(path).unwrap();
        assert!(contents.contains("world.update"));
